...
fn main() -> anyhow::Result<()> {
    ...
    let tmp_wav = format!("{TMP_WAV}_{:x}.wav", result);
    let tmp_asr_txt = format!("{tmp_wav}.txt");
    transcode(&url, &tmp_wav)?;
    match asr1(&mut trans, &tmp_wav) {
        Ok(txt) => {
            std::fs::write(&tmp_asr_txt, txt)?;
            println!("ASR result written to: {}", tmp_asr_txt);
        }
        Err(err) => {
            std::fs::write(tmp_asr_txt, format!("asr {url} {err}"))?;
            eprintln!("Error during ASR: {}", err);
        }
    }
